...
If you did everything correctly, `nvidia-smi` should show you the GPUs connected to your system.
...
This one's easy, just `yay -S docker docker-compose nvidia-container-toolkit`. Then, `systemctl enable docker` and `systemctl start docker` to make sure it runs and starts on boot. If you wanna avoid having to run all docker commands as root, add your user to the `docker` group with `sudo usermod -aG docker $USER`.

You can ensure everything worked with `docker run --rm --gpus all nvidia/cuda:12.1.1-base-ubuntu22.04 nvidia-smi`.

## 4. Kubernetes

[k3s](https://k3s.io/) seems to be the simplest way get kubernetes running, so that's what I went with. Just `curl -sfL https://get.k3s.io | sh -` and you're good to go (verify with `sudo kubectl get nodes`). If you want to run kubectl commands without sudo, `sudo cp /etc/rancher/k3s/k3s.yaml ~/.kube/config`.

### 4.2. DNS + SSL

Even though all traffic will be routed through Tailscale, I chose to set up a domain and SSL certificates to keep things pretty. Turns out Cloudflare makes getting SSL certs for non-public servers really easy, so keep that in mind if you choose to use something else.
...
While you don't need Rancher to run Dyson, I've found it makes k8s a little less scary by letting you click around instead of remembering a bunch of commands or writing tons of YAML. To install it, run `helm repo add rancher-latest https://releases.rancher.com/server-charts/latest && helm repo update && helm install rancher rancher-latest/rancher --namespace cattle-system --create-namespace --set hostname=rancher.yourdomain.com --set ingress.tls.source=secret --version v2.7.5-rc5 --set replicas=1 --set bootstrapPassword=YOUR_ADMIN_PASSWORD`. Then, generate the certificate with `RANCHER_DOMAIN="rancher.yourdomain.com" make rancher-cert`.

You should now be able to access Rancher at `https://rancher.yourdomain.com` from any machine in your tailscale network and log in with the password you set above.
